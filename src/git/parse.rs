...
impl Repo<'_> {
    pub fn parse_info(&mut self) {
        self.info_map = parse_info(&self.repo);
    }
}

pub fn parse_info(repo: &Repository) -> HashMap<PathBuf, FileInfo> {
    ...
    for step in revwalk {
        ...
        let mut diff = repo.diff_tree_to_tree(a.as_ref(), Some(&b), None).unwrap();
                ...
                (true, true) => {
                    ...
                    match status {
                        ...
                        FileStatus::Renamed(new_new_path) => {
                            status_map.insert(old_path, FileStatus::Renamed(new_new_path.clone()));
